PROGRAM ct_factor
IMPLICIT NONE
REAL*8 :: gridmin, gridmax, griddiff
REAL*8 :: kt0, kt, ktb, bias_fact, alpha_cv, deltaT, den
REAL*8 :: diff_s2, ds2, ss, hh, dum, num, gamma_
REAL*8, ALLOCATABLE :: cv(:), ht(:), ct(:), hill(:)
REAL*8, ALLOCATABLE :: grid(:), width(:), fes1(:)
...
DO i_md=1,md_steps
 READ(11,*) dum,dum,dum,dum,dum,dum,cv(i_md),dum   ! Rg10
END DO
...
DO i_s2=1,nbin     
   grid(i_s2)=gridmin+dfloat(i_s2-1)*griddiff
END DO
...
!        den=den+DEXP(-fes1(i_s2)/ktb)
         den=den+DEXP(-fes1(i_s2)/kt + fes1(i_s2)*gamma_/kt)
      END DO
