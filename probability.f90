...
DO i_md=1,md_steps
 READ(11,*) dum,cv1(i_md),cv3(i_md),cv2(i_md),cv4(i_md)
...
DO i_md=1,md_steps
 ...
 index4 = nint((cv4(i_md)-gridmin4)/griddiff4) +1
     ...
     ! i_mtd=(i_md*w_cv/w_hill)+1
      ...
      prob(index1,index2,index3,index4) = prob(index1,index2,index3,index4) + DEXP(dum/kt)
      den=den+DEXP(dum/kt)
     END IF
...
END DO
  ...
  den=den*griddiff1*griddiff2*griddiff3*griddiff4
   ...
   s2=DFLOAT(i_s2-1)*griddiff2+gridmin2
      ...
      s3=DFLOAT(i_s3-1)*griddiff3+gridmin3
         ...
         s4=DFLOAT(i_s4-1)*griddiff4+gridmin4
           ...
           prob(i_s1,i_s2,i_s3,i_s4)=prob(i_s1,i_s2,i_s3,i_s4)*(1.d0/den)
          
           WRITE(2)prob(i_s1,i_s2,i_s3,i_s4)
         END DO
      END DO
   END DO
END DO
